...
class SkyRCChannel(charger.Channel):
  ...
  def parseData(self, data):
    ...
    self.chargetime         = (data[3]*256) + data[4]
    self.voltage            = (data[5]*256) + data[6]
    self.current            = (data[7]*256) + data[8]
    self.temperature        = data[9]*10
    self.internal_temp      = data[10]*10
    if self.temperature == 0:
      self.temperature = self.internal_temp #This way we allways have a useful temperature
